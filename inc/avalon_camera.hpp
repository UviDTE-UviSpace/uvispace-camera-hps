...
private: //not accesible from ouside the class   
 ...
 //passed as an argument to this function. When all lines are aquired
 //the function ends.
 //
 ...
 int Camera::capture_get_line(cpixel*& line)
 {
    ...
    if (this->current_buff_v == this->buff0_v)
    {
        ...
        if ((IORD32(this->address, CAMERA_BUFF0_FULL)) == 1) return 1;
        else
        { 
    ...
    else
    {
        ...
        if ((IORD32(this->address, CAMERA_BUFF1_FULL)) == 1) return 1;
        else
        {
            //wait for the line to be acquired
